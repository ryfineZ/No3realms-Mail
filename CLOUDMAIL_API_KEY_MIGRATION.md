# Cloud Mail API Key 调用改造说明

本文档面向 Cloud Mail 接口调用方，说明如何从“登录 token 调用”改为“API Key 调用”。
...
这种方式可以继续使用，但不适合第三方项目高频调用。原因是 Cloud Mail 会限制同一用户的网页登录 token 数量。第三方项目如果频繁登录，会不断生成新 token，可能把网页登录状态挤掉，导致网页端需要重新登录。
...
由 Cloud Mail 管理员或对应账号登录 Cloud Mail 网页端：
...
async function cloudMailRequest(path, options = {}) {
  ...
  if (data.code !== 200) {
    throw new Error(data.message || 'Cloud Mail API 请求失败');
  }
...
API Key 的权限与创建它的 Cloud Mail 用户一致。
...
立即在 Cloud Mail 个人设置里删除该 API Key，然后创建新的 API Key，并更新调用方环境变量。
